...
namespace BulletPro
{
	...
	public class BulletModulePatterns : BulletModule
	{
		...
		// if this one emits bullets, they get stored here, but only while they're alive
		public List<Bullet> emittedBullets;

		// these ints are tracked for ammunition purposes, so they don't care if some bullets died
		public int totalShotsFired { get; private set; }
		public int totalBulletsEmitted { get; private set; }
		// the "recursive" counterparts take sub-emitters into account, in case fired bullets fire bullets.
		public int totalShotsFiredRecursive { get; private set; }
		public int totalBulletsEmittedRecursive { get; private set; }
		...
		// Returns how many bullets were actually emitted.
		public int ShootBullets(ShotParams sp, float timeOffset=0f)
		{
			if (!sp)
			{
			...
			for (int i = 0; i < realUsedBulletAmount; i++)
			{
				...
				Bullet b = EmitSingleBullet(bpToUse, spawn, minCoords, maxCoords, timeOffset);
				if (b != null) result++;
			}
			...
			while (cur != null)
			{
				cur.modulePatterns.totalShotsFiredRecursive++;
				cur = cur.subEmitter;
			}
		...
		public int ShootBullets(ShotParams sp, TempEmissionData ted, float timeOffset=0f)
		{
			solver.tempEmissionData = ted;
			return ShootBullets(sp, timeOffset);
		}

		// Init and emit another bullet. Returns it.
		public Bullet EmitSingleBullet(BulletParams bp, Vector3 delta, Vector2 minCoords, Vector2 maxCoords, float timeOffset=0f)
		{
			if (bp == null)
			{
			...
			while (cur != null)
			{
				cur.modulePatterns.totalBulletsEmittedRecursive++;
				cur = cur.subEmitter;
			}
			b.FirstPrepare(true);
			return b;
		...
		// For ammo tracking purposes: restart the count as if this emitter was just spawned.
		public void ResetEmissionCount()
		{
			totalShotsFired = 0;
			totalBulletsEmitted = 0;
			totalShotsFiredRecursive = 0;
			totalBulletsEmittedRecursive = 0;
		}
