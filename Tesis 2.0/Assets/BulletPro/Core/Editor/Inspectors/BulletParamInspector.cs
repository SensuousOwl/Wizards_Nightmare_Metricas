...
namespace BulletPro.EditorScripts
{
	...
	public class BulletParamInspector : EmissionParamsInspector
	{
		...
		SerializedProperty sprite, layer, sortingOrder, sortingLayerName;
		...
		void DrawRendererModuleInspector()
		{
			...
			if (isVisible.boolValue)
			{
				...
				layer.intValue = EditorGUILayout.LayerField("Layer (Postprocess)", layer.intValue);
				...
				else
				{
					...
					{
						int highest = 0;
						bool couldGetHighest = DynamicParameterUtility.GetHighestValueOfInt(evolutionBlendWithBaseColor, false, out highest);

						if (highest == (int)ColorBlend.Replace && couldGetHighest)
						{
