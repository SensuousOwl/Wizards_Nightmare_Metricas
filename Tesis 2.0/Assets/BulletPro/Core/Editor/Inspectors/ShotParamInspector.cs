...
namespace BulletPro.EditorScripts
{
	...
	public class ShotParamInspector : EmissionParamsInspector
	{
		...
		// Bullet spawn place
		SerializedProperty simultaneousBulletsPerFrame, bulletSpawnArray, hasUnknownBulletAmount, fakeBulletAmount;
		...
		public override void OnEnable()
		{
			...
			hasUnknownBulletAmount = serializedObject.FindProperty("hasUnknownBulletAmount");
			fakeBulletAmount = serializedObject.FindProperty("fakeBulletAmount");
		...
		public override void OnInspectorGUI()
		{
			...
			maxNumberOfBullets = 0;
			bool couldIdentifyNumberOfBullets = DynamicParameterUtility.GetHighestValueOfInt(simultaneousBulletsPerFrame, true, out maxNumberOfBullets);
			hasUnknownBulletAmount.boolValue = !couldIdentifyNumberOfBullets;
			if (!couldIdentifyNumberOfBullets)
			{
				EditorGUILayout.LabelField("Edit Layout : ??? bullets", EditorStyles.boldLabel);
				EditorGUILayout.HelpBox("Total number of bullets in this Shot could not be computed. This is most likely because your bullet amount is equal to another dynamic parameter.", MessageType.Info);
				maxNumberOfBullets = fakeBulletAmount.intValue;
			}
			...
			if (!couldIdentifyNumberOfBullets)
			{
				...
				EditorGUILayout.LabelField("Number of bullets to show in preview : "+fakeBulletAmount.intValue.ToString());
				if (GUILayout.Button("-5", GUILayout.MaxHeight(17))) fakeBulletAmount.intValue -= 5;
				if (GUILayout.Button("-1", GUILayout.MaxHeight(17))) fakeBulletAmount.intValue -= 1;
				if (GUILayout.Button("+1", GUILayout.MaxHeight(17))) fakeBulletAmount.intValue += 1;
				if (GUILayout.Button("+5", GUILayout.MaxHeight(17))) fakeBulletAmount.intValue += 5;
				EditorGUILayout.EndHorizontal();
				if (fakeBulletAmount.intValue < 0) fakeBulletAmount.intValue = 0;
				GUILayout.Space(-6);
			}
