...
namespace BulletPro.EditorScripts
{
    ...
    [CustomPropertyDrawer(typeof(DynamicIntValue))]
    public class DynamicIntValueDrawer : DynamicValueDrawer
    {
        public override void DrawFixed(SerializedProperty property, SerializedProperty nonDynamicValue, GUIContent label)
        {
            if (dynamicParameter.FindPropertyRelative("useSlider").boolValue)
                nonDynamicValue.intValue = EditorGUI.IntSlider(mainRect, label, nonDynamicValue.intValue, dynamicParameter.FindPropertyRelative("sliderMin").intValue, dynamicParameter.FindPropertyRelative("sliderMax").intValue);
            else
            {
                ...
                else
                {
                    float labelWidth = 80;
                    float space = 5;
                    float eatenWidth = labelWidth + 20;
                    float remainingWidth = mainRect.width - eatenWidth;
                    float btnWidth = (remainingWidth - ((float)buttons.arraySize-1f)*space)/(float)buttons.arraySize;
                    float curX = eatenWidth;

                    EditorGUI.LabelField(new Rect(mainRect.x, mainRect.y, labelWidth, mainRect.height), label.text+": "+nonDynamicValue.intValue.ToString()+". ");
