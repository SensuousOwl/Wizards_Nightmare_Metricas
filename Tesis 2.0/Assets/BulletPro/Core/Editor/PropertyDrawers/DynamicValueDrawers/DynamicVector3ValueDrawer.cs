using UnityEngine;
using UnityEditor;

// This script is part of the BulletPro package for Unity.
// Author : Simon Albou <albou.simon@gmail.com>

namespace BulletPro.EditorScripts
{
    // Base class for a DynamicParameter drawer related to the root struct. Heavily relies on inheritance.
    [CustomPropertyDrawer(typeof(DynamicVector3Value))]
    public class DynamicVector3ValueDrawer : DynamicVectorValueDrawer
    {
        public override void DrawFixed(SerializedProperty property, SerializedProperty nonDynamicValue, GUIContent label)
        {
            nonDynamicValue.vector3Value = EditorGUI.Vector3Field(mainRect, label, nonDynamicValue.vector3Value);
        }
    }
}