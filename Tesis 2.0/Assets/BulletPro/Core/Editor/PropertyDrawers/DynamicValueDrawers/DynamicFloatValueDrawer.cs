...
namespace BulletPro.EditorScripts
{
    ...
    [CustomPropertyDrawer(typeof(DynamicFloatValue))]
    public class DynamicFloatValueDrawer : DynamicValueDrawer
    {
        ...
        public override void DrawFixed(SerializedProperty property, SerializedProperty nonDynamicValue, GUIContent label)
        {
            if (dynamicParameter.FindPropertyRelative("useSlider").boolValue)
                nonDynamicValue.floatValue = EditorGUI.Slider(mainRect, label, nonDynamicValue.floatValue, dynamicParameter.FindPropertyRelative("sliderMin").floatValue, dynamicParameter.FindPropertyRelative("sliderMax").floatValue);
